 ...
 *   미허용 server(Q-01)·정상 성공·extractEvidence throw) + bridge-error 정규화, 그리고 각 분기의
 *   onOutcome CallOutcome 계측(F-02)을 확인한다.
 * @Author: shyang
...
function makeCtx(): { provided: unknown[] } {
  return { provided: [] };
}
...
describe('makeCallMcpTool', () => {
  ...
  it('(e) extractEvidence throw → raw result 유지, push 없음, success는 emit', async () => {
