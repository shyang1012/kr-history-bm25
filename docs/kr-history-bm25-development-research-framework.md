...
> 이 사례들은 **README 대표 사례 · 회귀 검증 픽스처**로 승격 대상이다 → `krh-8qi`.
>
> **회귀 자산화 원칙** — 역사적 *결론*이 아니라 도구의 *관찰 가능성*을 고정한다(확정 표현 금지 준수). 동봉 코퍼스 실측(2026-07-11) 결과 4표기 모두 재현되며, 이 중 3개(慈悲嶺·岊嶺·大同江·隴西)는 직역 미완 사서(kr·kj·ko) 소재라 **한자 인덱스(`search_han`)로만** 잡힌다 → 픽스처는 한자 기반이어야 하고, 이는 `trust-principle`과 정합한다. 이중 안전망: ① 알고리즘 진화(벡터·RRF·FDBSCAN) ② 보조(직역) 인덱스 확대 시 발견 가능성 보존.
...
> ※ `卽慈悲嶺` 명시 원문의 실재는 동봉 코퍼스 실측(kr_058 洞州 지리지)으로 확인됐고, 회귀 테스트 `tests/research/observability.e2e.test.ts`(krh-8qi)로 고정돼 있다.
